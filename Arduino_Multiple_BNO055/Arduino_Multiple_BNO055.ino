...
float angleX1 = 0.0, angleY1 = 0.0, angleZ1 = 0.0;
float angleX2 = 0.0, angleY2 = 0.0, angleZ2 = 0.0;

Adafruit_BNO055 sensor1 = Adafruit_BNO055(55, 0x28, &Wire); // Dirección por defecto
sensors_event_t orientationData1, angVelocityData1, linearAccelData1, magnetometerData1, accelerometerData1, gravityData1; //could add VECTOR_ACCELEROMETER, VECTOR_MAGNETOMETER,VECTOR_GRAVITY...

Adafruit_BNO055 sensor2 = Adafruit_BNO055(56, 0x29, &Wire); // Dirección modificada
sensors_event_t orientationData2, angVelocityData2, linearAccelData2, magnetometerData2, accelerometerData2, gravityData2;
...
void setup() {
  ...
  if (!sensor1.begin())
  {
    ...
    Serial.print("Ooops, no BNO055 detected ... Check your wiring or I2C ADDR 0x28!");
    while (1);
  }
  ...
  {
    /* There was a problem detecting the BNO055 ... check your connections */
    Serial.print("Ooops, no BNO055 detected ... Check your wiring or I2C ADDR 0x29!");
    while (1);
  }
...
void loop() {
  sensor1.getEvent(&orientationData1, Adafruit_BNO055::VECTOR_EULER); 
  angleX1 = orientationData1.gyro.x; // Velocidad angular en X
  angleY1 = orientationData1.gyro.y; // Velocidad angular en Y
  angleZ1 = orientationData1.gyro.z; // Velocidad angular en Z

  sensor2.getEvent(&orientationData2, Adafruit_BNO055::VECTOR_EULER); 
  angleX2 = orientationData2.gyro.x; // Velocidad angular en X
  angleY2 = orientationData2.gyro.y; // Velocidad angular en Y
  angleZ2 = orientationData2.gyro.z; // Velocidad angular en Z  
  // Crear objeto JSON
  ...
  if (central && central.connected()){
    ...
    if(calibrationChar.written() && imu_calibrated == false){
      ...
      if (calibration){
        Serial.println("Calibrando los sensores... Asegúrate de que el sensor está quieto.");
        calibrateSensor(sensor1); // Calibrar el sensor para determinar el offset
        calibrateSensor(sensor2); // Calibrar el sensor para determinar el offset
...
void calibrateSensor(Adafruit_BNO055 bno) {
